...
IRExpression* StmWrapper::ToExp() {assert(false);}

IRStatement* StmWrapper::ToStm() {
    return stm;
}

IRStatement* StmWrapper::ToCond(std::string&, std::string&) {assert(false);}

CondWrapper::CondWrapper(CJumpStatement::Relation rel, IRExpression* left, IRExpression* right): rel(rel), left(left), right(right) {}

IRExpression* CondWrapper::ToExp() {assert(false);}

IRStatement* CondWrapper::ToStm() {assert(false);}

IRStatement* CondWrapper::ToCond(std::string& t, std::string& f) {
    return new CJumpStatement(
        rel,
        left,
        right,
        t,
...
IRExpression* CondNotWrapper::ToExp() {assert(false);}

IRStatement* CondNotWrapper::ToStm() {assert(false);}
